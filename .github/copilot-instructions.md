...
- If the user asks for a commit, perform the following checks in order and only proceed when no blocking issues remain:
  ...
  3. **Tests**: Run `go test ./tests/...` and confirm all tests pass. Any failure is a blocking issue.
  4. **Doc Consistency**: Verify that `docs/architecture.md` (directory structure, routing table, design policy) and `README.md` accurately reflect the actual application code. Any discrepancy is a blocking issue.
  5. **Code Review**: Review the diff against the Code Review Rules above.
