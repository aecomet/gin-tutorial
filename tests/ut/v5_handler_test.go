...
// articleColumns はsqlmock用のカラム名リスト
...
func TestListArticles_OK(t *testing.T) {
	...
	countRow := sqlmock.NewRows([]string{"count"}).AddRow(2)
	mock.ExpectQuery(`SELECT count\(\*\) FROM`).WillReturnRows(countRow)
